...
Checkpoint("initialize A"); assert(sessionA.Initialize(publisherA,remoteA,subscriptionsA,config,[&](const uint8_t* d,std::size_t n){return sessionB.Feed(d,n);})); Checkpoint("initialize B"); assert(sessionB.Initialize(publisherB,remoteB,subscriptionsB,config,[&](const uint8_t* d,std::size_t n){return sessionA.Feed(d,n);}));
